<script>
    ...
    let id = $state("")
    //export let caravana = ""
    //export let connacimiento = false
    //export let peso = ""
    //export let sexo = "F"
...
<div class="mt-3 flex justify-start gap-2">
    {#if  !modoedicion}
        <button
            onclick={openEditar}
            class="btn text-lg px-6 py-2 bg-green-600 hover:bg-green-700 rounded-md text-white font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
        >
            Editar animal
        </button>    
    {:else}
...
<div class="justify-start">
    <button class="btn btn-error text-white" onclick={()=>goto("/animales")}>Volver</button>
</div>
