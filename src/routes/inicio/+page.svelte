    ...
    async function guardarNacimiento(){
        ...
        try{
            ...
            if(agregaranimal){
                ...
                let a = await guardarAnimal(true,false)
                await getAnimales()
                await pb.collection('animales').update(a.id,{nacimiento:recordparicion.id})
    ...
    async function guardarInseminacion(){
        if(agregaranimal){
            ...
            try{
                ...
                await pb.collection('animales').update(inseminacion.idanimalins,{
                    prenada:3
                })
                let recordinseminaciones = await pb.collection('inseminacion').getList(1,1,{
                    filter:`active=True && cab='${cab.id}'`
...
<Navbarr>
    ...
    {#if cab.exist}
        ...
        <CardBase titulo="Bienvenido a Creciente Fértil" cardsize="max-w-5xl">
            ...
            <h2 class="text-xl font-bold text-green-700 dark:text-green-400 mb-6 text-start">Acciones rapidas</h2>
...
<dialog id="nuevoModalTacto" class="modal modal-top mt-10 ml-5 lg:items-start rounded-xl lg:modal-middle">
    ...
    >
        ...
        <InicioTacto 
            ...
            bind:categoria 
            bind:agregaranimal 
            bind:tacto bind:prenadatacto 
            bind:madres
            bind:cargadoanimales {guardarTacto}>
        </InicioTacto>
...
<dialog id="nuevoModalNacimiento" class="modal modal-top mt-10 ml-5 lg:items-start rounded-xl lg:modal-middle">
    ...
    >
        ...
        <InicioNacimiento bind:listamadres bind:listapadres  bind:animales={animales} bind:nacimiento = {nacimiento} bind:cargadoanimales={cargadoanimales}  {guardarNacimiento} />
    </div>
</dialog>
<dialog id="nuevoModalTratamiento" class="modal modal-top mt-10 ml-5 lg:items-start rounded-xl lg:modal-middle">
    ...
    >
        ...
        <InicioTratamiento bind:tipotratamientos = {tipotratamientos}  bind:animales={animales} bind:tratamiento = {tratamiento} bind:cargadoanimales={cargadoanimales}  {guardarTrat}></InicioTratamiento>
